...
> **Date:** 2026-07-20
> **Companion doc:** For a TitanRL-only deep dive, see
> `torchtitan/experiments/rl/RL_EXPERIMENT_DEEP_DIVE.md` (and `RL_EXPERIMENT_SLIDES.md`).
> This document is the **three-way comparison** of TitanRL vs. Slime vs. Miles.
>
> **Scope:** A theory-and-implementation comparison of three open-source RL
> post-training stacks for LLMs — **TitanRL** (the `experiments/rl` subproject
> inside PyTorch **torchtitan**), **Slime** (THUDM), and **Miles** (RadixArk) —
> across scope, cross-functional (XFN) footprint, target audience,
> implementation/architecture, feature surface, and performance characteristics.
>
> **Method:** All three repos read directly (source, READMEs, docs, configs).
> Slime & Miles were shallow-cloned from GitHub; TitanRL was read from this local
> torchtitan checkout at `torchtitan/experiments/rl/`. **No GPU runs were executed**
> (this machine has no GPU). TitanRL perf figures cited here come from a prior
> independent 8×H100 evaluation of this same experiment.
>
...
>
> **Versions analyzed:** TitanRL @ torchtitan `main` (c95b211bc); Slime @ `ea9819f`;
> Miles @ `93a2137`.
...
- **TitanRL** is a *correctness-first reference RL loop* living inside **PyTorch
  torchtitan**. Its thesis is **correctness through a single, unified model
  definition**: the exact same torchtitan model class is used by both the trainer
  (torchtitan/FSDP) and the generator (vLLM), enabling **bitwise-identical**
  train/inference logprobs. It is small (~12K LOC), research-grade,
  single-node-friendly, and built on Meta's **Monarch** (actor orchestration) +
  **TorchStore** (RDMA weight sync) + **vLLM**. Think "the correctness-first,
  hackable RL substrate for the PyTorch ecosystem."
...
- **Slime** is a *production-validated, SGLang-native* RL post-training framework
  ...
  reinforce each other**: high-performance **training** (Megatron) + flexible
  **data generation** (SGLang), joined by a **Data Buffer**. It is the RL framework
  behind the **GLM-4.5 → GLM-5.2** model family — battle-tested at frontier scale
...
ecosystem (PyTorch-native, vLLM, Monarch), a different primary goal (numerical
correctness / reference implementation rather than max-scale production
throughput), and a much smaller, younger, experimental codebase.
...
| **Approx. core LOC (Python)** | ~11,950 | ~29,300 | ~46,600 |
...
- Ships **GRPO** and **DAPO** losses (GRPO = DAPO with symmetric clip). Single-turn
  ...
  rollouter.
- Two headline examples: `alphabet_sort` (toy smoke task) and `search_r1`
  (multi-turn retrieval-augmented QA with EM reward).
...
| **Model definition** | **Unified** — one torchtitan model class for BOTH train & infer | Separate: Megatron for train, SGLang for infer, bridged via mbridge / megatron_to_hf | Same as Slime + FP8/INT4 aligned conversion paths |
...
FP8-unify + R3 routing replay + TIS/MIS are all about closing the train-inference
mismatch that a two-stack design creates).
...
- Fully **async** pipeline of asyncio loops; back-pressure governed by a
...
| VLM (vision) | ⚠️ (LLM-focused; VLM caveats noted) | ✅ (geo3k_vlm) | ✅ unified VLM/LLM multi-turn |
...
- **Consumer**: PyTorch-ecosystem researchers who want an authoritative, hackable
  RL loop that "just matches" between train and infer.
...
> this machine; this is a theory + code comparison). What follows is (a) TitanRL's
> measured numbers from a prior 8×H100 evaluation of this experiment, and (b) each
> project's *stated* performance posture from code/docs/blogs.

### 7.1 TitanRL — measured (Qwen3-8B, Search-R1, 8×H100, TP2/TP2, on-policy)
Batch-invariant (BI) mode ON vs OFF, per-step medians:
...
retrieval + multi-turn orchestration + straggler sync. On a compute-bound task the
~2.5× would surface. The payoff is *exact* generator/trainer agreement (logprob
diff = 0 every step).
...
- Validated by **full GLM-4.5→5.2 post-training** — the strongest possible "it
  scales and converges" signal. Verified on MoE up to ~355B.
...
- **INT4 QAT**: fits **1TB-scale** models into single-machine VRAM (H200),
  ~doubling rollout efficiency by eliminating cross-node bottlenecks while claiming
  BF16-equivalent accuracy.
- **Unified FP8 & R3**: aimed at *correctness at scale* — eliminate the
  quantization-induced and MoE-routing-induced train/inference mismatch that causes
  RL collapse in large MoE (Qwen3, DeepSeek-V3).
...
### TitanRL (explicit TODOs in code / docs)
...
- Weight-sync can OOM-spike on large models (GPU-Direct default; CPU staging TODO).
- Base loop is **single-turn**; multi-turn only via the Search-R1 rollouter.
- Only GRPO/DAPO (no PPO-critic, no RLHF reward model, no KL penalty); no pipeline
  parallelism in the trainer; partial resume only.
- Depends on **nightly** torch + vLLM (cu130) + Monarch/TorchStore — fragile,
  fast-moving install surface. "APIs and configurations may change."

### Slime
- **SGLang-only** rollout backend by design (want vLLM? use the `vime` fork).
- Megatron-only training (no native FSDP path).
- Two-stack model design → train/infer parity is a managed concern, not a guarantee.
...
- **TitanRL**: `torchtitan/experiments/rl/{README.md, train.py, controller.py,
  ...
  companion `RL_EXPERIMENT_DEEP_DIVE.md` / `RL_EXPERIMENT_SLIDES.md`.
- **Slime** (`ea9819f`): `README.md`, `train.py`, `train_async.py`, `pyproject.toml`,
  `requirements.txt`, full `slime/` + `slime_plugins/` module trees; lmsys.org vision
  blog; thudm.github.io/slime docs.
- **Miles** (`93a2137`): `README.md`, `pyproject.toml`, `requirements.txt`,
  `.gitmodules`, full `miles/` module tree incl. `router/`, `true_on_policy/`,
  `backends/experimental/fsdp_utils/`; `docs/{advanced,user-guide}`; RadixArk / lmsys
  blogs (FP8, INT4-QAT, R3, launch announcement).
